...
Runs four feature sets for 24-ROI and 50-ROI baselines on ALL / COARSE / FINE
(class subsets) for all subjects.

Feature sets
-----------

1. gamma24:
   • 24 × gamma power (mean-squared of 70–150 Hz band-passed signal) — ALL 24 ROIs
   • +8 × targeted line length on original broadband time series — V1, V2, Fusiform, IT (bilateral)

2. ll24:
   • 24 × line length on original broadband time series — ALL 24 ROIs
   • +8 × targeted gamma power (70–150 Hz) — V1, V2, Fusiform, IT (bilateral)

3. gamma50:
   • 50 × gamma power (mean-squared of 70–150 Hz band-passed signal) — ALL 50 ROIs (intersection across trials)
   • +8 × targeted line length on original broadband time series — V1, V2, Fusiform, IT (bilateral; only ROIs present)

4. ll50:
   • 50 × line length on original broadband time series — ALL 50 ROIs (intersection across trials)
   • +8 × targeted gamma power (70–150 Hz) — V1, V2, Fusiform, IT (bilateral; only ROIs present)
...
OUT_FEAT_DIR = HERE / "gamma_ll_baselines_features"
OUT_RES_DIR  = HERE / "gamma_ll_baselines_results"
OUT_PLOT_DIR = HERE / "gamma_ll_baselines_plots"
...
VISUAL_LL_ROIS = ['V1-lh','V1-rh','V2-lh','V2-rh','Fusiform-lh','Fusiform-rh','IT-lh','IT-rh']
...
FEATURE_SETS = ["gamma24", "ll24", "gamma50", "ll50"]
...
def gamma_power(ts: np.ndarray, fs: int, band: Tuple[float,float], win: Tuple[float,float],
                log_power: bool = False) -> float:
    y = bandpass(ts, band[0], band[1], fs)
    seg = _slice_window(y, fs, win)
    if seg.size == 0:
        return 0.0
    val = float(np.mean(seg ** 2))
    if log_power:
...
def build_features_for_rows(
    ...
    """
    Generic feature builder for a list of trials rows with given ROI names.

    Returns (X, y, feature_names) where y is class label (string).
    """
    X_list: List[List[float]] = []
    y_list: List[str] = []

    # Determine baseline ROI set and targeted visual ROIs for this set
    visual_rois_present = [r for r in VISUAL_LL_ROIS if r in roi_names]

    for row in rows:
        ts = extract_roi_timeseries(row["stc_filename"], row["roi_selections"], roi_names)
        feats: List[float] = []

        if feature_set in ("gamma24", "gamma50"):
            # Baseline: gamma power for all ROIs
            for r in roi_names:
                idx = roi_names.index(r)
                val = gamma_power(ts[idx], FS, gamma_band, win, log_power=log_power)
                feats.append(val)
            # Targeted LL on broadband for visual ROIs
            for r in visual_rois_present:
                idx = roi_names.index(r)
                val = line_length_broadband(ts[idx], FS, win)
                feats.append(val)

        elif feature_set in ("ll24", "ll50"):
            # Baseline: LL on broadband for all ROIs
            for r in roi_names:
                idx = roi_names.index(r)
                val = line_length_broadband(ts[idx], FS, win)
                feats.append(val)
            # Targeted gamma power for visual ROIs
            for r in visual_rois_present:
                idx = roi_names.index(r)
                val = gamma_power(ts[idx], FS, gamma_band, win, log_power=log_power)
                feats.append(val)
        ...
        X_list.append(feats)
        # For all/coarse we use class_name; for fine groups we will pass class_code instead
    ...
    feat_names: List[str] = []
    if feature_set in ("gamma24", "gamma50"):
        feat_names.extend([f"{r}_gP" for r in roi_names])
        feat_names.extend([f"{r}_LLbb" for r in visual_rois_present])
    else:
        feat_names.extend([f"{r}_LLbb" for r in roi_names])
        feat_names.extend([f"{r}_gP" for r in visual_rois_present])
...
def run_kfold_rf(X: np.ndarray,
                 ...
                 seed: int = 42) -> Dict:
    """
    5-fold stratified CV with RF (gini criterion).
...
def run_all_coarse_for_subject(
    ...
    roi_set = 24 if feature_set in ("gamma24", "ll24") else 50
    ...
    X, y, feat_names = build_features_for_rows(
        rows, roi_names, feature_set, win, gamma_band, log_power=log_power
    )
...
def run_fine_for_subject(
    ...
    for gname, codes in FINE_GROUPS.items():
        ...
        # For fine groups we use class_code labels for y
        for r in rows:
...
def main():
    ap = argparse.ArgumentParser(
        description="Gamma vs line-length baselines (24- and 50-ROI) on all/coarse/fine class subsets."
    )
    ap.add_argument("--subjects", type=str, default="all",
                    help="Subject ids, e.g. 'all', '0-15' or '0,1,2'")
    ap.add_argument("--feature-sets", type=str,
                    default="gamma24,ll24,gamma50,ll50",
    ...
    ap.add_argument("--gamma-low",  type=float, default=70.0)
    ap.add_argument("--gamma-high", type=float, default=150.0)
    ap.add_argument("--log-power", action="store_true")
    ...
    gamma_band = (args.gamma_low, args.gamma_high)
    ...
    print(f"Window={win} | gamma={gamma_band[0]}-{gamma_band[1]} Hz | log_power={args.log_power}")
    ...
    if summary_rows:
        ...
        summary_csv = OUT_RES_DIR / f"summary_all_subjects_{subjects[0]}-{subjects[-1]}.csv"
